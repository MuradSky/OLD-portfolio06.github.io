...
module.exports = function serve(cb) {
    ...
    gulp.watch('src/blocks/**/*.scss', gulp.series(styles, cb => gulp.src('build/css').pipe(server.stream()).on('end', cb)))
    gulp.watch('src/js/*.js', gulp.series(script, readyReload))
    gulp.watch('src/pages/*.pug', gulp.series(pug2html, readyReload))
    gulp.watch('src/blocks/**/*.pug', gulp.series(pug2html, readyReload))
